...
(function ctrlblk_cssProcedural() {
/******************************************************************************/
...
class PSelectorVoidTask extends PSelectorTask {
    constructor(task) {
        super();
        console.info(`ctrlblk: :${task[0]}() operator does not exist`);
    }
    transpose() {
    }
}
...
class ProceduralFilterer {
    ...
    ctrlblk_commitNow() {
        ...
        for ( const pselector of this.selectors.values() ) {
            ...
            if ( pselector.budget < -500 ) {
                console.info('ctrlblk: disabling %s', pselector.raw);
    ...
    styleTokenFromStyle(style) {
        ...
        ctrlblk_injectCSS(`[${this.masterToken}][${styleToken}]\n{${style}}\n`);
        return styleToken;
    }
