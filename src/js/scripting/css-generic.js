...
const ctrlblk_idFromNode = (node, out) => {
    const raw = node.id;
...
const ctrlblk_classesFromNode = (node, out) => {
...
const ctrlblk_processNodes = ( ) => {
    ...
    for (;;) {
        ...
        for ( const node of nodes ) {
            ctrlblk_idFromNode(node, styleSheetSelectors);
            ctrlblk_classesFromNode(node, styleSheetSelectors);
        }
        nodes.length = 0;
    ...
    styleSheetTimer = self.requestAnimationFrame(( ) => {
        ...
        ctrlblk_injectCSS(`${styleSheetSelectors.join(',')}{display:none!important;}`);
        styleSheetSelectors.length = 0;
    });
};

/******************************************************************************/
const ctrlblk_processChanges = mutations => {
    ...
    processTimer = self.setTimeout(( ) => {
        ...
        ctrlblk_processNodes();
    }, 64);
};
/******************************************************************************/

const ctrlblk_injectCSS = (css, count = 10) => {
...
ctrlblk_processNodes();
let domMutationObserver = new MutationObserver(ctrlblk_processChanges);
...
const stopAll = reason => {
    ...
    console.info(`ctrlblk: Generic cosmetic filtering stopped because ${reason}`);
};
