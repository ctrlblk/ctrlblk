...
import { processFilterLists } from './engine.js';
    ...
    ruleset = ruleset.map((rule) => {
        if (rule.condition?.urlFilter || rule.condition?.regexFilter) {
            if (rule.condition.isUrlFilterCaseSensitive === undefined) {
                ...
                // Lowercase urlFilter when case-insensitive
...
async function generateFourPieceScriptlet(scriptlet, mapin, flatArgsList) {
    ...
    for (let [selector, details] of mapin) {
        if (!details.rejected) {
            if (!details.matches || details.matches.length === 0) continue;
...
async function commitScriptlets(rulesetId, dirPath) {
    ...
    for (const [name, details] of scriptletFiles) {
        ...
        await writeFile(`${dirPath}/${rulesetId}.${name}`, content);
...
async function main({ output }) {
    ...
    for (let ruleset of rulesets) {
        ...
        await writeFile(`${rulesetDir}/main/${ruleset.id}.json`, dnr.src);
        ...
        for (let [filename, func, args] of scriptlets) {
            ...
            if (src) {
                ...
                await writeFile(`${rulesetDir}/scripting/${filename}`, src);
            }
        }
...
const parser = new ArgumentParser({ description: "Convert easylist style filter list to DNR+" });
