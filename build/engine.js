// Core engine for converting EasyList filter lists to Chrome DNR rulesets.
...
function findExtendedSeparator(line) {
    // Use a regex similar to reExtAnchor to detect extended filter separators
    // Matches: ##, #@#, #?#, #@?#, #$#, #@$#, #$?#, #@$?#
    const re = /(#@?(?:\$\?|\$|%|\?)?#)/;
...
// This mirrors normalizeHostnameValue which replaces * with __asterisk__
...
// in the path part. Mirrors normalizePattern behavior.
...
function parseDomainList(domainStr) {
    ...
    for (const { hn, not } of parts) {
        ...
        if (domain.length > 2 && domain.charCodeAt(0) === 0x2F && domain.charCodeAt(domain.length - 1) === 0x2F) {
            // Regex domain pattern — normalize using RegExp.source (matches normalizeRegexPattern)
            try {
                const source = domain.slice(1, -1);
                const regex = new RegExp(source);
                domains.push({ hn: `/${regex.source}/`, not, bad: false });
            } catch (e) {
                // Invalid regex — mark as error (matches AST_FLAG_HAS_ERROR)
                hasError = true;
            }
            continue;
        }
        ...
        // reBadHostnameChars rejects [\x00-\x24\x26-\x29\x2b\x2c\x2f\x3b-\x40\x5c\x5e\x60\x7b-\x7f]
        // If any hostname has bad chars, the entire filter is rejected (matches hasError() check)
    ...
    // If any domain is invalid, reject the entire filter
    // (addToDNR checks parser.hasError() before addExtendedToDNR)
    if (hasError) return null;
    return domains;
}

// ============================================================================
// CSS selector normalization (match AST-based serialization)
// ============================================================================
...
// Normalize a CSS selector to match the AST serialization:
...
function normalizeCssSelector(sel) {
    ...
    while (i < sel.length) {
            ...
            if (!afterAttrOp) {
                ...
                if (opLen > 0) {
                    ...
                    function collectFlags() {
                        ...
                        if (trimmed === '') return '';
                        // astSerializePart validates flags with /^(is?|si?)$/
        ...
        if (cc === 0x28) { // (
            ...
            // (the AST serializer strips leading whitespace inside :has(), :not(), etc.)
            if (isSelectorCtx) {
                while (i < sel.length && sel.charCodeAt(i) === 0x20) i++;
            }
            continue;
        ...
        if (!inNonSelectorParen && (cc === 0x2B || cc === 0x3E || cc === 0x7E)) { // + > ~
            ...
            // In that case, don't add space before the combinator
    ...
    // The AST serializer always uses explicit coefficient: n+B → 1n+B, -n+B → -1n+B
    result = result.replace(/:(nth-(?:child|of-type|last-child|last-of-type))\((-?)n\+/g, ':$1($21n+');

    // Trim leading whitespace (the AST serializer doesn't produce leading spaces)
...
// csstree accepts these in the AST but astSerializePart rejects them
...
function compileSelector(selector, nativeCssHas) {
    ...
    // Pre-normalize ABP pseudo-class names (match parser behavior)
    ...
    // These are marked as errors and discarded
    if (/:(-abp-)/.test(selector)) {
        return null;
    }

    // Capture raw selector after ABP normalization but before :contains() normalization
    // and CSS normalization. The OLD engine stores this as the "raw" field on
    ...
    // Normalize operator aliases (match normalizedOperators)
    ...
    // astSerializePart rejects them with reInvalidIdentifier = /^\d/)
...
// Action pseudo-classes produce "action" key instead of "tasks" (match ActionSelector)
const actionPseudoClasses = new Set(['remove', 'remove-attr', 'remove-class', 'style']);

// Operator name normalization (matches normalizedOperators)
...
function compileProcedural(selector, match, nativeCssHas) {
    ...
    // The AST serializer adds * for the implicit type selector
    ...
    // csstree compilation fails when :is() wraps procedural content
    ...
    if (actionPseudoClasses.has(pseudoClass)) {
        ...
        // :style is cssable when there are no tasks, or only matches-media tasks (match isCssable)
    ...
    // matches isCssable(): matches-media qualifies, with no action or :style action
...
function compileProceduralArgs(name, args, nativeCssHas) {
    ...
    case 'if': {
        ...
        // Plain CSS selector - :if() returns bare string (matches compileArgumentAst)
    ...
    case 'has': {
        ...
        // Plain CSS selector - :has() wraps in object (matches compileArgumentAst)
    ...
    case 'not': {
        ...
        // Plain CSS selector - returned as-is for :not() (matches compileArgumentAst)
    ...
    case 'upward':
        ...
        return normalizeCssSelector(args);
    case 'nth-ancestor':
        // nth-ancestor is normalized to upward
...
// Compile :not() argument - astCompile returns string or object
function compileNotArg(args, nativeCssHas) {
    let normalized = normalizeCssSelector(args);
    return normalized;
}

// Strip outer quotes from a string (matches unquoteString)
...
// Compile text argument (matches compileText → unquoteString)
...
// compileCSSDeclaration: extracts pseudo, name, and value.
// Non-regex values are escaped and wrapped in ^...$
function compileCssDeclarationArg(name, arg) {
    // matches-css-before/after prepend "before, " / "after, "
    let argStr = arg.trim();
...
function parseNetworkOptions(optionsStr) {
    ...
    // end of the option string (it's always the last option in filter syntax).
    ...
    for (let option of optionsStr.split(',')) {
        ...
        // $redirect-rule is rejected at parser level (badTypes includes
        ...
        // Options that require trusted source:
        ...
        case 'urlskip':
            ...
            break;
        // $cname: blocking filters are rejected by the parser (realBad = true when
        ...
        // The filter compiles and produces a type bit entry (bit position 12),
        ...
        // Negated (~inline-font): sets NOT_TYPE_BIT which causes
        ...
        case 'mp4':
            ...
            break;
        // strict1p/strict3p can't be implemented in DNR —
        // FilterStrictParty.dnrFromCompiled explicitly rejects these.
        // Unlike parser-rejected options, these compile to full error rules.
        case 'strict1p': case 'strict3p':
            opts._strictParty = key;
            break;
        // $header=value: sets HEADER_BIT which causes the filter to be
        // rejected from DNR output (FilterFromCompiled.dnrFromCompiled returns undefined).
        // The filter compiles but produces no output (no rule, no error).
        // $header (no value): the parser rejects this (hasError), producing bare error.
        case 'header':
        ...
        default:
            ...
            if (/^_+$/.test(key)) break;
            // Unknown options are unsupported - unrecognized options are rejected
            opts._unsupported = true;
            break;
        }
    }
    ...
    // types. The filter compiles but produces no output (no rule, no error).
...
// In the OLD engine, a single source filter line can produce multiple
...
// Check if a filter is "just-origin" (isJustOrigin()).
...
function isJustOriginFilter(pattern, opts) {
    ...
    // (checks /[\/~]/.test(this.fromDomainOpt))
    ...
    // Anchor must be exactly 0b010 (left anchor only, not ||)
...
function computeTypeBitCount(opts) {
    ...
    // generichide is type 15 (bit 14), specifichide is type 16 (bit 15)
...
function splitNetworkFilter(line) {
    ...
    // Find option separator ($) using the indexOfNetAnchor algorithm.
    ...
    if (dollarIdx >= 0 && dollarIdx < line.length) {
...
function indexOfNetAnchor(s) {
    // Matches AstFilterParser.indexOfNetAnchor
    const end = s.length;
    if (end === 0) return end;
    let j = s.lastIndexOf('$');
    if (j === -1) return end;
...
// ============================================================================
// Bare hostname detection (from reHostnameAscii)
// Matches patterns like "example.com", "46.8.158.80" (no filter syntax)
// ============================================================================

// Case-insensitive: patterns are normalized to lowercase before this check
...
function extractHostnameOnly(urlFilter) {
    ...
    // (matching FilterAnchorHn behavior)
...
// Check if parsed options have "option units".
// The DOT_TOKEN_HASH path (pure hostname → requestDomains) only fires
...
function networkFilterToDNR(pattern, options, exception, extensionPaths) {
    ...
    // Skip patterns with whitespace (classified as malformed hosts entries
    ...
    // bare error matching FILTER_UNSUPPORTED behavior).
    ...
    if (urlFilter.startsWith('/') && urlFilter.endsWith('/') && urlFilter.length > 2) {
        ...
        // Normalize regex pattern using RegExp.source (matches normalizeRegexPattern).
        ...
        // RE2 compatibility check (matches sfp.utils.regex.isRE2).
        ...
        // Lowercase non-regex patterns (matching normalizePattern behavior)
        urlFilter = urlFilter.toLowerCase();
        // Track whether original pattern has wildcards (before any stripping)
        // This is checked on the raw pattern and used to gate trailing wildcard stripping
        ...
        // Matches rePointlessLeadingWildcards = /^(\*+)[^%0-9A-Za-z\u{a0}-\u{10FFFF}]/u
        ...
        // rePointlessTrailingWildcards = /(?:[^%0-9A-Za-z]|[%0-9A-Za-z]{7,})(\*+)$/
        ...
        // Note: the full ignoreLen is always stripped regardless of needWildcard
        ...
        if (hnOnly && (hnOnly.hasCaret || hasAnyFilterOptions(options) || exception)) {
            ...
            // Normalize scheme patterns: isJustOrigin/rulesetFromData
            ...
            // |http*:// and |http*: match both http and https — these are treated
    ...
    if (options.all) {
        ...
        // However, if negated non-DNR types are present (e.g., ~inline-font), the
    ...
    // The parser rejects the entire filter when a domain contains * in a
    ...
    // isJustOrigin() filters with the same (realm, party, type, tokenHash)
    ...
    if (initInclude.length > 0) {
        ...
        // ALL just-origin filters in the same (action, party, token,
    ...
    // Track if the original filter had domain options (FROM_BIT).
    // Even after entity domain removal, this affects token hash classification:
    // FROM_BIT prevents DOT_TOKEN_HASH and ANY_TOKEN_HASH.
    ...
    // matching MODIFY_BIT which prevents DOT/ANY classification.
    // This is critical for error rules where action.type may not be set to 'redirect'
    // or 'modifyHeaders', but the filter IS a modifier in the compilation.
    ...
    if (options.redirect !== null || options.redirectRule !== null) {
        if (options.redirectRule !== null) {
            ...
            // These are internally converted to block rules with higher priority,
            ...
            if (extensionPath) {
                ...
                // Even when the redirect token is unresolvable, the action type
        ...
        if (exception) {
            ...
            // They patch the matching block removeparam rules' conditions.
            ...
            // Typeless modifier rules stay in the no_type bucket during
        ...
        if (exception) {
            ...
            // CSP filters with no explicit type get doc+frame types added
            ...
            if (!options.types.length && !options.negatedTypes.length && !options.all) {
                ...
                // CSP types are added via processTypeOption BEFORE
        ...
        if (exception) {
            ...
            // Like CSP, permissions filters without explicit types get doc+frame
            if (!options.types.length && !options.negatedTypes.length && !options.all) {
                rule._errorMultiplier = 2;
            }
        } else {
    ...
    // Important flag: in the DNR code, $important block rules (BLOCKIMPORTANT_REALM)
    ...
    // General error multiplier: compileToAtomicFilter creates one
...
function finalizeRuleset(rules, invalidFilters, bareInvalidFilters) {
    ...
    // isJustOrigin merging: isJustOrigin() filters with the same
    ...
    for (const rule of rulesetMap.values()) {
        if (rule._error === undefined) {
            ...
            rule.id = 0;
            // Error multiplier: compileToAtomicFilter creates per-type
    ...
    // context.invalid collects error messages from filters rejected
...
function parseScriptletArgs(content) {
    ...
    // Parse arguments using the same logic as ArgListParser:
    ...
    while (pos < len) {
        ...
        if (qc === 34 || qc === 39 || qc === 0x60) { // " ' `
            // Try quoted argument - find matching close quote (match ArgListParser)
...
// Remove escape backslashes before a specific character (normalizeArg)
...
// Convert AdGuard scriptlet name to the equivalent scriptlet name
function convertAdGuardScriptletName(adgName) {
    // AdGuard names: 'set-constant', 'abort-on-property-read', etc.
    // Names are the same but suffixed with .js in the build output
    // The names are identical between AdGuard and the engine for most scriptlets
    return adgName;
}

// ============================================================================
// Normalize a filter line for $badfilter matching.
// Badfilter matching is done against compiled representations where aliases are
...
function extractPerDomainBadfilter(normalizedLine) {
    ...
    // Normalize pattern: `*` and empty are equivalent (ANY_TOKEN_HASH)
...
function computeBadfilterKey(normalizedLine) {
    ...
    // Normalize pattern: both `*` and empty pattern compile to
...
export async function processFilterLists(lists, options = {}) {
    ...
    for (const list of lists) {
        ...
        // Multi-domain filters are compiled to per-domain atomic entries,
        ...
        for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
            ...
            // Malformed hosts-file entries: when the parser sees a line
            ...
            // that match reHostnameAscii are converted to ||hostname^
            ...
            // A multi-domain filter $3p,domain=a.com|b.com|c.com is compiled into
            ...
            // (e.g., "matichon*.com"). The parser sets AST_FLAG_HAS_ERROR,
            ...
            if (isDuplicate || skipCounting) {
                ...
                // Just-origin filters: each (domain, type) pair is a
                ...
                // Normalize pattern to token hash category: `*`, ``,
            ...
            // Silent skip: options that compile but produce no output.
            ...
            // cosmetic filter suppression entry, no network DNR rule.
            ...
            if (exception && (opts.generichide || opts.elemhide)) {
                // Collect generichide exclusion domains. Exclusions
                ...
                if (!opts.firstParty && !opts.thirdParty) {
                    ...
                    if (ghFromDomains.length > 0) {
                        ...
                        // FilterAnchorHn converts pure hostname patterns
                        // to requestDomains before to= domains are appended.
                        // The parser classifies a pattern as hostname if it
                ...
                // for the remaining types. compileToAtomicFilter creates
            ...
            // compiler.compile() returns FILTER_UNSUPPORTED for genericblock,
            ...
            // When isPureHostname && hasNoOptionUnits(), the hostname goes
            ...
            // $strict1p/$strict3p: these compile fully but are marked
            ...
            // main_frame is a non-network type (type offset 14, beyond
            ...
            if (opts.all && !rule._error && !rule.condition.regexFilter) {
                ...
                // (hasNoOptionUnits()). Filters with option units
                // (FROM_BIT, NOT_TYPE_BIT, etc.) are EMPTY_TOKEN_HASH;
                ...
                // and these filters are ANY_TOKEN_HASH which pre-merges.
                    ...
                    // compileToAtomicFilter creates main_frame entries for
    ...
    // compileToAtomicFilter writes one entry PER type bit for all
    ...
    // the wrong type group, breaking category ordering within type buckets.
    {
        ...
        for (let i = 0; i < dnrRules.length; i++) {
            ...
            // compileToAtomicFilter creates per-type entries for all
            ...
            // Error rules are also split: even filters that produce
            ...
            if (needsSplit) {
                for (const type of rt) {
                    ...
                    // Per-type compiled entries go into separate buckets,
    ...
    // In toRuleset, DOT_TOKEN_HASH entries are grouped by
    ...
    // position and accumulates domains; later occurrences are removed.
    {
        ...
        for (let i = 0; i < dnrRules.length; i++) {
            ...
            // prevents DOT_TOKEN_HASH classification.
            // FROM_BIT (domain= option) prevents DOT_TOKEN_HASH, even
    ...
    // In toRuleset, ANY_TOKEN_HASH/ANY_HTTPS/ANY_HTTP entries are
    ...
    // with all domains combined, matching the per-type bucket accumulation.
    {
        ...
        for (let i = 0; i < dnrRules.length; i++) {
            ...
            // entries and must NOT be pre-merged.
    ...
    // Sort rules to match the realm-based iteration order:
    ...
    // Realm iteration order: BLOCK → ALLOW → REDIRECT → REMOVEPARAM → CSP → PERMISSIONS
    ...
    // Classify each rule into a "token hash category" matching the bucket grouping:
    ...
    const getTokenCategory = (rule) => {
        ...
        // keep action.type='block' but ARE modifiers in the compilation).
        ...
        // isPureHostname && hasNoOptionUnits().
        ...
        // ANY: isJustOrigin — optionUnitBits === FROM_BIT
        ...
        // These filters are still compiled with ANY_TOKEN_HASH
    ...
    // This determines the category ordering within each group (matches Map insertion order).
    // For each group, find the minimum original index for each token category
    // This determines the category ordering within each group (matches Map insertion order)
    ...
    // during sorting, matching the behavior where modifier post-processing
    ...
    // These are collected separately in context.responseHeaderRules and
...
function processExtendedFilter(ext, originalLine, trustedSource, nativeCssHas,
    ...
    if (type === 'adguard') {
        ...
        if (!left) return;
        if (exception) {
            // Validate domain before skipping: invalid hostnames cause
        ...
        // Convert AdGuard scriptlet name to the equivalent name
    ...
    if (type === 'css') {
        ...
        if (braceIdx === -1) {
            ...
            // These are rejected at the parser level producing a bare "Rejected filter" error.
            if (bareInvalid) bareInvalid.add(`Rejected filter: ${originalLine}`);
            return;
        }
        ...
        // Transform like translateAdguardCSSInjectionFilter
        if (/^\s*remove:\s*true[; ]*$/.test(style)) {
            // {remove:true} → :remove()
    ...
    if (type === 'cosmetic') {
        ...
        if (exception) {
            ...
            if (!hasOptions) {
                ...
                // Invalid hostnames (e.g., containing /) cause parser.hasError()
                // which produces a bare "Rejected filter" before reaching extended filter handling.
                const domains = parseDomainList(left);
                if (!domains) {
                    if (bareInvalid) bareInvalid.add(`Rejected filter: ${originalLine}`);
                } else {
                    // Domain-specific exceptions add domains to excludeMatches
                    addSpecificCosmetic(specificCosmeticFilters, compiled, domains, true);
                }
            }
            return;
        }
        ...
        if (!compiled) {
            // Invalid CSS selectors cause parser.hasError() which
